...
def replace_nan_with_mean( ds, variable='RAINFALL'):
    ...
    ds[variable] = ds[variable].fillna(mean_value)  # Replace NaNs
    return ds
...
# Merge all seasonal datasets into one (ensure time alignment)
merged_dataset = xr.concat(season_datasets, dim="TIME")
...
ds_andhra = ds_andhra.sortby("TIME")
